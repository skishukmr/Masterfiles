...
1       Nagendra       28/01/2009      Created
2       Nikita(1140)   01-06-2010      Changed the details from Preparer to Requester.

******************************************************************************/

package config.java.workflow.sap;
...
public class CatSAPNotifySupplierOnNRContractOpen extends Action {

	private static final String ClassName = "CatSAPNotifySupplierOnNRContractOpen";

	private static String EmailTestMode = ResourceService.getString("cat.email.sap", "Contract_NotificationInTestMode");
	private static String EmailTestModeAddress = ResourceService.getString("cat.email.sap", "Contract_NotificationInTestModeAddress");
	private static String EmailBCCAddresses = ResourceService.getString("cat.email.sap", "Contract_NotificationBCCList");
...
//	private static final String LEGAL_ENTITY_NG = ResourceService.getString("cat.java.vcsv3","PO_LegalEntity_NG");
	...
	private class EmailHelperMethods {
		...
		{
			String emailSubject = Fmt.Sil(suppLocale, "cat.email.sap", "Contract_NotificationEmailSubject");
		...
		String generateEMailBody(Contract ma, SupplierLocation supLoc, Locale suppLocale)
		{
			...
			String emailTextContractNum = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationContractNum");
			String emailTextContractDesc = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationContractDesc");
			String emailTextContactName = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationContactName");
			String emailTextContactPh = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationContactPh");
			String emailTextSupp = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationSupplier");
			String emailTextSuppPh = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationSuppPh");
			String emailTextSuppFx = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationSuppFx");
			String emailTextBillTo = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationBillTo");
			String emailTextBillToPh = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationBillToPh");
			String emailTextBillToFx = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationBillToFx");
			String emailTextShipTo = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationShipTo");
			String emailTextShipToPh = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationShipToPh");
			String emailTextShipToFx = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationShipToFx");
			String emailTextLegalEntity = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationLegalEntity");
			String emailTextItemInfo = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationItemInfo");
			String emailTextItem = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationItem");
			String emailTextDesc = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationDescription");
			String emailTextUOM = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationUOM");
			String emailTextUOMAbbrv = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationUOMAbbrev");
			String emailTextSuppRefNum = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationSuppRefNum");
			String emailTextUOMNote = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationUOMNote");
			//String emailTextNote1 = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationNote1");
			//String emailTextNote2 = Fmt.Sil(suppLocale,"cat.email.sap", "Contract_NotificationNote2");
			String emailTextterms = Fmt.Sil(suppLocale,"cat.email.sap", "PO_TermsAndConditionscontract");
			...
			Log.customer.debug("%s :::partUserReq : \n%s", ClassName, partUserReq);
			String comdesc = (String)mar.getDottedFieldValue("CompanyCode.Description");
			if(comdesc!=null)
				emailTextLegalEntity = emailTextLegalEntity + comdesc;
			Log.customer.debug("%s :::emailTextLegalEntity : \n%s", ClassName, emailTextLegalEntity);
			...
			//-- start Changed by Nikita for issue number 1140
			if (partUserReq != null) {
				emailTextContactName = emailTextContactName + partUserReq.getName().getPrimaryString();
				emailTextContactPh = emailTextContactPh + partUserReq.getDottedFieldValue("DeliverToPhone");
			}
			//-- End
			if (supLoc != null) {
			...
			/*Address billToBasic = partUserReq.getBillingAddress();
			Log.customer.debug("%s :::billToBasic : \n%s", ClassName, billToBasic);
			...
			Log.customer.debug("%s :::billTo : \n%s", ClassName, billTo);
			Address shipToBasic = partUserReq.getShipTo();
			Log.customer.debug("%s :::shipToBasic : \n%s", ClassName, shipToBasic);
			...
			Log.customer.debug("%s :::shipTo : \n%s", ClassName, shipTo);*/
			ariba.common.core.Address billTo =(ariba.common.core.Address) mar.getDottedFieldValue("CompanyCode.DefaultBillToAddress");
			Log.customer.debug("%s :::billTo : \n%s", ClassName, billTo);
			ariba.common.core.Address shipTo =(ariba.common.core.Address) mar.getDottedFieldValue("ContractShipTo");
			Log.customer.debug("%s :::shipTo : \n%s", ClassName, shipTo);
			...
			if (shipTo != null) {
				...
				String shiptoAddress = shipTo.getPostalAddress().getLines() + "\n" +
				shipTo.getPostalAddress().getCity() + ", " +
				shipTo.getPostalAddress().getState() + " - " +
				shipTo.getPostalAddress().getPostalCode();

				emailTextShipTo = emailTextShipTo + "\n" + shiptoAddress;
				emailTextShipToPh = emailTextShipToPh + shipTo.getPhone();
				emailTextShipToFx = emailTextShipToFx + shipTo.getFax();
			}
			else {
				shipToAvail = false;
			}
			...
			emailText += ResourceService.getString("cat.email.sap", "PO_TermsAndConditionsMsgcontract")+emailTextterms + "\n\n";
	...
	public CatSAPNotifySupplierOnNRContractOpen() {
		super();
	}
}