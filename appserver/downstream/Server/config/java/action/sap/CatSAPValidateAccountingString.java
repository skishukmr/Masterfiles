...
=====================================================================================================================================
Bycyrus	 Purush Kancharla	May-15-2012		 Code added to include CGM condition to return WBS element as null for Account Category F
	325  IBM_AMS_Vikram		Sept-12-2012     Add a new account type S under requisitions for MACH1 5.0 release
	     IBM Niraj Kumar    Jan-28-2013      Mach1 R5.5 (FRD12.2/TD12.2) Added logic for Cost center blocked from Posting
*************************************************************************************************************************************/
...

{
...

    {
        ...
        if(object instanceof ProcureLineItem || object instanceof ReqLineItem )
...

                    {
...

                        {
...

                            {
								...
								// Start: Mach1 R5.5 (FRD12.2/TD12.2)                                
                             else if (response.getResultCode().equals("022"))    
                                {
                                	Log.customer.debug("Response Code returned for MACH1 5.5 for blocked cost center");
                                	Log.customer.debug("Response Code returned for MACH1 5. Returnig response.getResultCode" +response.getResultCode());
                                    StringBuffer sb022 = (new StringBuffer(InvalidAccountingMsg)).append(response.getMessage()).append(AdditionalMessage);
                                    sa.setFieldValue("ValidateAccountingMessage", sb022.toString());
                                    Log.customer.debug("Response Code returned for MACH1 5. Returnig response.getResultCode" +response.getMessage());        				                
                    				if (!fp.isImmutable()) {
                                        fp.setPropertyForKey("ValueStyle", "catRedTextSm");
                    				}
								}
                               // End: Mach1 R5.5 (FRD12.2/TD12.2)
                              else
                                {
     ...
     /* if (company!=null && sapsrc.equalsIgnoreCase("MACH1") && acccategory!=null)
        ...
        if (company!=null && sapsrc.equalsIgnoreCase("MACH1") && acccategory!=null)
        {
         ...
         if (acccat.equalsIgnoreCase("F"))
		   ...
		   else if (company!=null && sapsrc.equalsIgnoreCase("CGM") && acccategory!=null)
		   {
		   Log.customer.debug("Its a CGM Company Code, checking the Account Category");
         acccat = (String)acccategory.getDottedFieldValue("UniqueName");
         if (acccat.equalsIgnoreCase("F"))
            {
           Log.customer.debug (" Account Category for a CGM company Code is F, hence return a WBS element as null");
            wbsele = null;
           }
		   }
    ...
    public boolean isAccountValidationRequired(SplitAccounting sa, ProcureLineItem pli){
		...
		// Issue 325: Start (Vikram added below else if block to allow new Acct type S for accounting validation)
