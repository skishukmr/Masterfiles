...
public class CatValidCostCenterForUser extends Condition {

	private static final String classname = "CatValidCostCenterForUser";
		...
		if (object instanceof SplitAccounting){
			...
			if (li instanceof ReqLineItem && costcenter != null) {
				...
				if (testuser != null) {
					...
					if (CCapprover == null) {
						Log.customer.debug("%s *** CostCenter approver is null", classname);
				    ...
				    {
				    	Log.customer.debug("%s *** CostCenter approver is ", CCapprover.getUniqueName());
						ariba.common.core.User partrequester = ariba.common.core.User.getPartitionedUser(requester, lic.getPartition());
				    	if(CCapprover.equals(partrequester)){
						reason = 2;
				    	result = false;
				    	}
				    }
	...
	public CatValidCostCenterForUser() {
		super();
	}
