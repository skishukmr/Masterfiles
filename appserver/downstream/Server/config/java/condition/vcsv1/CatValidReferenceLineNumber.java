/* Created by KS on Sep 21, 2005
 * --------------------------------------------------------------
 * Used to validate Reference Line No. set by user for Additional Charge lines
 * 04.25.06 (ks) Prevent referencing a material line below the AC (needed for order splitting)
 * 05.22.06 (ks) Prevent referencing a deleted material line on a revision (V2+)
 */
...
public class CatValidReferenceLineNumber extends Condition {

	private static final String THISCLASS = "CatValidReferenceLineNumber";
    ...
    public boolean evaluate(Object object, PropertyTable params) throws ConditionEvaluationException {
        ...
        Log.customer.debug("CatValidReferenceLineNumber *** errorCode: " + errorCode);
        return errorCode==0;
    }
    ...
    public ConditionResult evaluateAndExplain(Object object, PropertyTable params)
	    ...
	    if(!evaluate(object, params)) {
			...
			return new ConditionResult(ResourceService.getString("cat.java.vcsv1",errorMsg));
		}
		return null;
    }
    ...
    public static int validReferenceLine (ProcureLineItem acLine) {
        ...
        if (refNumInt != null) {
	        ...
	        if (CatAdditionalChargeLineItem.isAdditionalCharge(acLine)) {
	            if (refNum == 0) {
	                ...
	                if (plic != null) {
	                    ...
	                    else if (CatAdditionalChargeLineItem.isAdditionalCharge(matLine))
        ...
        Log.customer.debug("CatValidReferenceLineNumber *** code: " + code);
        return code;
    }


	public CatValidReferenceLineNumber() {
		super();
	}
