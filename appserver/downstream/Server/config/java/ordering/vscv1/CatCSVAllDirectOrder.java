 ...
 * 10.11.13 Abhishek Kumar - Q4 2013 - RSD111 - FDD 3.0/TDD 1.3 Logic added to pull the legal entity from Ship To Name in Order print.
 *	15/01/2014     IBM Parita Shah	SpringRelease_RSD 111(FDD4.8/TDD1.8) MSC Tax Gaps Correct Legal Entity
 */
...
public class CatCSVAllDirectOrder extends AllDirectOrder
{
        ...
        throws OrderMethodException
    {
		...
		// Starts SpringRelease_RSD 111(FDD4.8/TDD1.8)
         String AccountingFac = (String)req.getFieldValue("AccountingFacilityName");
         Log.customer.debug("AccountingFacilityName on REQ is:",AccountingFac);
         // Ends SpringRelease_RSD 111(FDD4.8/TDD1.8)
        ...
        for (int i=0; i<listsize; i++) {
            ...
            Log.customer.debug("%s *** \n   PO#: %s, UniqueName: %s \n",THISCLASS, po.getUniqueId(),newPOUniqueName);
           	     //Start: Q4 2013 - RSD111 - FDD 3.0/TDD 1.3
	     POLineItem pli = (POLineItem) po.getLineItems().get(0);
            String shipToname=null;
            if (pli != null && pli.getShipTo() != null){
            shipToname = pli.getShipTo().getName();
	     Log.customer.debug("%s SHIPTO Name Test *** %s",THISCLASS,shipToname);
            }
            // set PO header fields
            po.setFieldValue("EmergencyBuy", emergencyBuy);
            //po.setFieldValue("LegalEntity", LEGAL_ENTITY);

            //End: Q4 2013 - RSD111 - FDD 3.0/TDD 1.3

            // Starts SpringRelease_RSD 111(FDD4.8/TDD1.8)
            if(!StringUtil.nullOrEmptyOrBlankString(AccountingFac))
            {
				Log.customer.debug("Setting Accounting Facility Name on PO Header");
				po.setFieldValue("AccountingFacilityName",AccountingFac);
				po.setFieldValue("LegalEntity", AccountingFac);
			}
			// Ends SpringRelease_RSD 111(FDD4.8/TDD1.8)
