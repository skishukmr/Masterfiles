...
public class CatEZOInvoiceReconciliation_Print extends StatementReconciliation_Print {

    private static final String THISCLASS = "CatEZOInvoiceReconciliation_Print";
 ...
 //		     runPrintHook((Approvable)bo,out,userFields,out,locale);
    ...
    protected void printHTMLLineItems(LineItemCollection lic, PrintWriter out, Locale locale) {
        ...
        if (!lic.getLineItems().isEmpty()) {
            CatEZOInvoiceReconciliationLineItem_Print catIRlip = new CatEZOInvoiceReconciliationLineItem_Print();
            catIRlip.printHTMLLineItems(out,lic,lic.getLineItems(),lic.getTotalCost(),locale);
        }
    }
    public CatEZOInvoiceReconciliation_Print() {
        super();
    }
