...
public class DWInvoiceProcess extends ScheduledTask
{
    ...
    private static final String THISCLASS = "####DWInvoiceProcess#### ";
		...
		try
        {
			...
			while(results.next())
			{
				...
				if (invrecon.getFieldValue("MasterAgreement") != null)
				{
					...
					{
						ariba.base.core.Log.customer.debug("%s ....%s", THISCLASS, "The BuyerCode : " + buyerresults.getObject(0) );
                    	cr = (ClusterRoot) buyerresults.getBaseId("BuyerCode").get();
                    	if (cr != null)
                    		ariba.base.core.Log.customer.debug("%s ....%s", THISCLASS, "Buyer for MA IS " + cr);
                    	else
                    		ariba.base.core.Log.customer.debug("%s ....%s", THISCLASS, "Buyer for MA IS NULL");
					}
				}
				...
				{
					//Setting FacilityFlag, PONumber, POCreateDate, POLineItemNumber, BuyerCode
					...
					if (doinvreconli.getFieldValue("Order") != null)
					{
						...
						ariba.base.core.Log.customer.debug("%s Populating......POCreateDate", THISCLASS);

						ariba.base.core.Log.customer.debug("%s Populating......ExchangeRate", THISCLASS);
					}
					else
					{
				...
				for (int i=0; i< invrecon.getLineItemsCount(); i++)
				{
					...
					if (doinvreconli.getFieldValue("Order") != null)
					{
						order = (ariba.purchasing.core.DirectOrder)doinvreconli.getFieldValue("Order");// Below fields commented out for CR216
						...
						if (doinvreconli.getDottedFieldValue("OrderLineItem") != null)
							doinvreconli.setFieldValue("POLineItemNumber", (Integer)doinvreconli.getDottedFieldValue("OrderLineItem.NumberInCollection") );
						else
					...
					{
						ariba.base.core.Log.customer.debug("%s For MA POLineItemNumber was %s", THISCLASS, (Integer)doinvreconli.getFieldValue("POLineItemNumber"));
						doinvreconli.setFieldValue("POLineItemNumber", null);
						ariba.base.core.Log.customer.debug("%s For MA POLineItemNumber After Update %s", THISCLASS, (Integer)doinvreconli.getFieldValue("POLineItemNumber"));
						if (cr!= null)
						{
