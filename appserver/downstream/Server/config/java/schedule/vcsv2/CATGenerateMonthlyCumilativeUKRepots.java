...
/*
 * This class is used to run the the monthly cumulative UK Reports for 18 months
 *  Lekshmi - Issue 1171- Scheduled task for generating UK Monthly cumulative Reports
 */
public class CATGenerateMonthlyCumilativeUKRepots extends ScheduledTask {

	private String thisclass = "CATGenerateMonthlyCumilativeUKRepots";
	private int dayOfWeek = 0;
	private int dayOfMonth = 0;
	private AQLDateLiteral aqlCumulativeFromDate = null;
	private AQLDateLiteral aqlCumulativeToDate = null;
	private Date cumulativeFromDate = null;
	private Date cumulativeToDate = null;
	Hashtable monthlyCumulativehash = new Hashtable();
	...
	public void init(Scheduler scheduler, String scheduledTaskName,
		...
		setHashFromResourceFile("cat.monthlyukcumilativereport.util",
				monthlyCumulativehash);
		Log.customer.debug("%s overrideDates: " + overrideDates, thisclass);
	}

	private int getLastDayOfMonth() {
		Log.customer.debug("Getting Last Day of the curentMonth");
	...
	public void run() throws ScheduledTaskException {
		...
		Date cumulativeReportTo = Date.getNow();
		Log.customer.debug("cumulative end date", cumulativeReportTo);
		Date cumulativeReportFrom = Date.getNow();
		cumulativeReportFrom.addMonths(cumulativeReportFrom, -18);
		Log.customer.debug("cumulative start date", cumulativeReportFrom);
		DateRange cumulativeDateRange = new DateRange(Base.getSession()
				.getPartition(), cumulativeReportFrom, cumulativeReportTo);
		aqlCumulativeFromDate = new AQLDateLiteral(cumulativeDateRange
				.getFromDate());
		aqlCumulativeToDate = new AQLDateLiteral(cumulativeDateRange
				.getToDate().getNow());
		monthlyMethod();
	}

	public void monthlyMethod() {
		Enumeration keylist = (Enumeration) monthlyCumulativehash.keys();
		for (Enumeration e = keylist; e.hasMoreElements();) {
			List reportlist = (List) monthlyCumulativehash.get(e.nextElement());
	...
	private boolean sendReportToday(List reportList, String reportType) {
		...
		if (overrideDates)
			return true;
		if (reportType.equals(MONTHLY) && (dayOfMonth == daytorunreport))
			return true;
	...
	public void executequeryForKeyValuePair(List reportList) {
		...
		for (Iterator e = reportList.iterator(); e.hasNext();) {
			...
			String query = Fmt.S(reportLine[QUERY], aqlCumulativeFromDate,
					aqlCumulativeToDate);
	...
	private String getDefaultMessage() {
		return ResourceService.getService().getString(
				"cat.monthlyukreport.util", "MsgBody");
	}
