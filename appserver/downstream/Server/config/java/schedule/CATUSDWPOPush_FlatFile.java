/*******************************************************************************************************************************************
   ...
   Madhavan Chari
   Date : 26/05/2008
   Writting Shipto.Receving Facility
   ...
   pgskannan
   Date : 2/17/2008
   change : issue 898 - Mail subject corrected.
...
public class CATUSDWPOPush_FlatFile  extends ScheduledTask  {
    ...
    private String classname = "CATUSDWPOPush_FlatFile";
    ...
    private String flatFilePath = "/msc/arb9r1/downstream/catdata/DW/MSC_DW_PO_PUSH_US_"+fileExt+".txt";
    //change made by soumya begins

	private String archiveFileDataPath = "/msc/arb9r1/downstream/catdata/DW/archive/MSC_DW_PO_PUSH_US_ARCHIVE"+fileExt+".txt";
    ...
    public void run() throws ScheduledTaskException  {
        ...
        usPOFlagAction = ResourceService.getString("cat.dwAction", "USPOFlagAction");
        ...
        try {
    		...
    		mailSubject = "CATUSDWPOPush_FlatFile Task Completion Status - Completed Successfully";

            try {

                query = "select from ariba.purchasing.core.DirectOrder where DWPOFlag in ('InProcess','Processing') and StatusString in ('Ordered','Receiving','Received')";
                ...
                while (results.next()) {
                    ...
                    if (directOrder != null) {
                        try {
							...
							// 2 Source-Of-Record Facility  IF PARTITION = PCSV1 THEN U9
							 outPW_FlatFile.write("U9" + "~|");
							 Log.customer.debug("%s::Source-Of-Record Facility:%s",classname,"U9");
							...
							//4 PH-Buying-Facility-Code  IF Partition = PCSV1 THEN "U9"
							outPW_FlatFile.write("U9" + "~|");
							Log.customer.debug("%s::PH-Buying-Facility-Code:%s",classname,"U9");

							//5 PH-Receiving-Facility-Code IF Partition = PCSV1 LineItems. ShipTo. ReceivingFacility
...
//**********************************************
							//19 PL-Account-Facility-Code IF Partition = PCSV1 THEN LineItems. Accountings.SplitAccountings.AccountingFacility
							if (sa.getFieldValue("AccountingFacility") !=null){
							String pLAccountFacilityCode = sa.getFieldValue("AccountingFacility").toString();
							outPW_FlatFile.write(pLAccountFacilityCode + "~|");
							Log.customer.debug("%s::pLAccountFacilityCode:%s",classname,pLAccountFacilityCode);
							}
							else {	outPW_FlatFile.write("~|");	}

							 //20 PL-Control-Account  LineItems. Accountings. SplitAccountings.Department
							    if (sa.getFieldValue("Department") != null) {
								 //String pLControlAccount = sa.getDottedFieldValue("CostCenter.CostCenterCode").toString();
								 String pLControlAccount = sa.getFieldValue("Department").toString();
							...
							//21 PL-Sub-Acct  IF Partition = PCSV1 THEN LineItems. Accountings.SplitAccountings. Division
							if (sa.getFieldValue("Division") != null) {
							String pLSubAcct = sa.getFieldValue("Division").toString();
							...
							//22 PL-Sub-Sub-Acct IF Partition = pcsv1 THEN IR.LineItems. Accountings.SplitAccountings.Section
							if ( sa.getFieldValue("Section") != null) {
							String pLSubSubAcct = sa.getFieldValue("Section").toString();
							...
							//23 PL-Expense-Account IF Partition = PCSV1 THEN LineItems. Accountings.SplitAccountings.ExpenseAccount

							if (sa.getDottedFieldValue("ExpenseAccount") != null) {
							String pLExpenseAccount = sa.getDottedFieldValue("ExpenseAccount").toString();
							outPW_FlatFile.write(pLExpenseAccount + "~|");
							Log.customer.debug("%s::pLExpenseAccount:%s",classname,pLExpenseAccount);
							}
							else { outPW_FlatFile.write("~|");	}

							//24 PL-Order-Number IF Partition = PCSV1 THEN LineItems. Accountings.SplitAccountings.Order
							...
							//25 PL-Misc-Qualifier IF Partition = PCSV1 THEN LineItems. Accountings.SplitAccountings.Misc
							...
							//26 IF Partition = PCSV1 THEN LineItems. Accountings. SplitAccountings. ExpenseAccount

							if (sa.getFieldValue("AccountDescription") != null) {
							  String pLExpenseAccountDesc = sa.getFieldValue("AccountDescription").toString();
							...
							//28 PL-Central-Source-Facility-Code IF PARTITION = PCSV1 THEN "U9"
							String pLCentralSourceFacilityCode = "U9";
						...
						// End :  SpringRelease_RSD 131 (FDD_131_4.6 / TDD_131_1.7)
							...
							/*
							if (flushCount == 50) {
								Log.customer.debug(" ###### Object count is 50 Need to flush  ########",classname);
								outPW_FlatFile.flush();
								flushCount =0;
								int balance = resultCount - pushedCount;
								Log.customer.debug("Balance need to write balance  "+balance +" Records",classname);
							}
						 ...
						 if (tmpDWPOFlag == "Processing")
						 {
							Log.customer.debug("%s::DWPOFlag is before setting to Completed in the if loop :%s", classname,tmpDWPOFlag);
						...
						Log.customer.debug("%s::After setting DWPOFlag to completed transaction commiting", classname);
						 ...
						 if(count == 25)
						{
							 Log.customer.debug("**********Commiting DO DW Records for count 25*******  ",count);
...
//          add message
			...
			*/
			...
			Log.customer.debug("CATUSDWPOPush_FlatFile:Starting Copying the flat file to Archive ");
			CATFaltFileUtil.copyFile(flatFilePath, archiveFileDataPath);
			Log.customer.debug("CATUSDWPOPush_FlatFile:Completed Copying the flat file to Archive ");

			try
			{
				Log.customer.debug("CATUSDWPOPush_FlatFile:Changing file permission of Data file.");
				Runtime.getRuntime().exec("chmod 666 " + flatFilePath);
				Log.customer.debug("CATUSDWPOPush_FlatFile:Changed file permission of Data file.");
			}catch (IOException e1) {
				Log.customer.debug("CATUSDWPOPush_FlatFile:Error in changing Permission. "+ e1);
			}
